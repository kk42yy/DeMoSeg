...
def generate_result(R0, R1, R2):
    R = []
    for x, y, z in zip(R0, R1, R2):
        cur = [x[0], x[1]]
        for i in range(len(x)-2):
            cur.append(np.round(np.nanmean(np.array((x[i+2],y[i+2],z[i+2]))), 4))
        R.append(cur)
    return R
...
if __name__ == "__main__":
    miss_result_bath_dir_path = '...'
    post = True
    RFNet_Order = True
    
    savepath = miss_result_bath_dir_path + (f'/Miss_pp_result_otherep.csv' if post else '/Miss_result_otherep.csv')
    RESULT0 = []
    for i in range(15):
        case_csv_result_path = miss_result_bath_dir_path + '/fold_0/missing_modality/' + (f'missing_{i}_post' if post else f'/missing_{i}') + '/Eval_Sta.csv'
        RESULT0.append(read_csv(case_csv_result_path, i))
    
    RESULT1 = []
    for i in range(15):
        case_csv_result_path = miss_result_bath_dir_path + '/fold_1/missing_modality/' + (f'missing_{i}_post' if post else f'/missing_{i}') + '/Eval_Sta.csv'
        RESULT1.append(read_csv(case_csv_result_path, i))
    
    RESULT2 = []
    for i in range(15):
        case_csv_result_path = miss_result_bath_dir_path + '/fold_2/missing_modality/' + (f'missing_{i}_post' if post else f'/missing_{i}') + '/Eval_Sta.csv'
        RESULT2.append(read_csv(case_csv_result_path, i))
    
    RESULT = generate_result(RESULT0, RESULT1, RESULT2)
