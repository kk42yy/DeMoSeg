...
def producer(queue: Queue, data_loader, batch_size, transform, thread_id: int, seed,
...
class MultiThreadedDataLoader(object):
    ...
    def __init__(self, data_list, batch_size, transform, num_processes, num_cached=2, seeds=None, pin_memory=False,
                 wait_time=0.02):
        ...
        self.generator = deepcopy(data_list)
        ...
        if not self.initialized:
            ...
            for i in range(self.num_processes):
                self._processes.append(Process(target=producer, args=(
                    self._queue, self.generator, self.batch_size, self.transform, i, self.seeds[i], self.abort_event, self.wait_time
                )))
