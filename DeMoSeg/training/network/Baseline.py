...
from training.network.ConvBlock import ConvDropoutNormNonlin, Upsample
...
class Baseline(nn.Module):
    def __init__(self,
        ...
        self.conv_blocks_context.append(nn.Sequential(
            ConvDropoutNormNonlin(self.input_channels, self.features_per_stage[0]),
            ConvDropoutNormNonlin(self.features_per_stage[0], self.features_per_stage[0])
        ))
        for in_fea, out_fea in zip(self.features_per_stage[:-1], self.features_per_stage[1:]):
            self.conv_blocks_context.append(nn.Sequential(
                ...
                ConvDropoutNormNonlin(out_fea, out_fea)
            ))
        ...
        for in_fea, up_out_fea in zip(self.features_per_stage[::-1][:-1], self.features_per_stage[::-1][1:]):
            ...
            self.conv_blocks_localization.append(nn.Sequential(
                ...
                ConvDropoutNormNonlin(up_out_fea, up_out_fea)
            ))
